...
namespace MediaPlayer_X_Ark.Engine.Visualize
{
	...
	public class WaveformAnalyzer
	{
		...
		/// <summary>
		/// 非同期で波形解析を実行する（既存 Sound オブジェクト版）。
		/// CDDA 等の OPENMEMORY/CREATESAMPLE サウンド向け。Sound の所有権は移譲しない。
		/// </summary>
		public Task AnalyzeFromSoundAsync(
			FMOD.Sound sound,
			PlayList entry,
			Action<PlayList> onCompleted,
			CancellationToken ct = default)
		{
			return Task.Run(() =>
			{
				try
				{
					AnalyzeCore(sound, entry, ct, releaseOnFinish: false);
					if (!ct.IsCancellationRequested)
						onCompleted?.Invoke(entry);
				}
				catch (OperationCanceledException) { }
				catch (Exception e) { System.Diagnostics.Debug.WriteLine($"[WaveformAnalyzer] {e}"); }
			}, ct);
		}
		...
		private void Analyze(string filename, PlayList entry, CancellationToken ct)
		{
			...
			AnalyzeCore(sound, entry, ct, releaseOnFinish: true);
		}

		/// <summary>
		/// 波形解析の本体。ファイル版・Sound版共通ロジック。
		/// </summary>
		/// <param name="releaseOnFinish">true のとき解析後に sound.release() を呼ぶ（ファイル版のみ）。</param>
		private void AnalyzeCore(FMOD.Sound sound, PlayList entry, CancellationToken ct, bool releaseOnFinish)
		{
			try
			{
				...
				// CDDA 等で entry.LengthMs が未設定の場合はSoundから補完
				if (entry.LengthMs == 0 || entry.LengthMs == 0xFFFFFFFF)
				{
					sound.getLength(out uint lengthMs, FMOD.TIMEUNIT.MS);
					if (lengthMs > 0) entry.SetLength(lengthMs);
				}
			...
			{
				if (releaseOnFinish && sound.hasHandle())
					sound.release();
			}
		}

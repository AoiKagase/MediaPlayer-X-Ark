...
namespace MediaPlayer_X_Ark
{
	...
	class PlayerEngine
	{
		...
		protected void FmodCallFunction(FMOD.RESULT result)
        {
		...
		protected void Initialize()
		{
			...
			{
				// Get Version.
				FmodSystem.getVersion(out FmodVersion);
				...
				// System Init.
				FmodSystem.init(channelCount, FMOD.INITFLAGS.NORMAL, IntPtr.Zero);

				// Create Channel Group.
				FmodSystem.createChannelGroup("Channel 01", out FmodChannelGroup);

				spectrum = new FmodSpectrum(ref FmodSystem, 512, ref this.FmodChannelGroup);
			}
			else
            {
				lastError = FMOD.Error.String(result);
        ...
        {
			bool paused;
			FmodChannel.getPaused(out paused);
		...
		public void SetOutputType(FMOD.OUTPUTTYPE outputtype)
        {
			...
			if (FmodOutputType != outputtype)
            {
				...
				FMOD.RESULT result = FmodSystem.setOutput(outputtype);
				lastError = FMOD.Error.String(result);
			}
		}
        ...
        {
			FmodSystem.getOutput(out FmodOutputType);
        }
        ...
        public void GetDeviceList()
        {
			...
			for (int i = 0; i < numDrivers; i++)
            {
				...
				device.namelen = 64;
				device.deviceId = i;
				FmodSystem.getDriverInfo(i, out device.name, device.namelen, out device.guid, out device.systemrate, out device.speakermode, out device.speakerModeChannels);
        ...
        {
			FmodSystem.playSound(FmodSound, FmodChannelGroup, false, out FmodChannel);
        }
		...
		public void CreateSound(string filename)
        {
            ...
            {
				FmodCallFunction(FmodSystem.createStream(filename, FMOD.MODE.DEFAULT | FMOD.MODE.NONBLOCKING, out FmodSound));
			}
        }
